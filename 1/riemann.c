...
void Riemann(double *U1, double *U4, double *F) {
  ...
  if (revflag) {
    if (s4 > 0) {
      ...
      f1 = -rho*u;
      f2 = rho*u*u + p ;
      f3 = -0.5*rho*u*u*u - rho*a*a*u/(gamma-1.);
    }

    else if (s2 > 0) {
      f1 = -rho3*u3;
      f2 = rho3*u3*u3 + p3;
      f3 = -0.5*rho3*u3*u3*u3 - rho3*a3*a3*u3/(gamma-1.);
    }

    else if (s1 > 0) {
      f1 = -rho2*u2;
      f2 = rho2*u2*u2 + p2;
      f3 = -0.5*rho2*u2*u2*u2 - rho2*a2*a2*u2/(gamma-1.);
    }

    else {
      f1 = -rho1*u1;
      f2 = rho1*u1*u1 + p1;
      f3 = -0.5*rho1*u1*u1*u1 - rho1*a1*a1*u1/(gamma-1.);
    }
  }
